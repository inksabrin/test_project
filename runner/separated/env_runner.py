...
class EnvRunner(Runner):
    ...
    def collect(self, step):
        ...
        for agent_id in range(self.num_agents):
            ...
            action_space_name = action_space.__class__.__name__
            
            if action_space_name == "MultiDiscrete":
                for i in range(action_space.shape):
                    uc_action_env = np.eye(action_space.high[i] + 1)[action[:, i]]
                    if i == 0:
                        action_env = uc_action_env
                    else:
                        action_env = np.concatenate((action_env, uc_action_env), axis=1)
            elif action_space_name == "Discrete":
                action_env = np.squeeze(np.eye(action_space.n)[action], 1)
            else:
